...
**A rolling wave — packets specified with the user one at a time** (the user, 2026-09-25, for a wave of small fixes). The user describes a problem; you reproduce it on `origin/dev` and propose the packet — the behaviour it must have, the test that shows it red, its boundary, and whether it corrects code against a row or changes a decided behaviour (then a new D# row through a converge round, never an edit of the old one) — and close it with `AskUserQuestion`. The agreed packet takes the next task id in the plan's task table. From there it is an ordinary packet: every rule of §3–§7 binds it unchanged. What differs is only how the wave grows:
- The plan lands with its first packet and says in its frontmatter that it is rolling (`rolling: true`); later packets are added while it is `claimed`.
- Each later packet is one `knowledge/` pull request adding a dated `## Packet <T#> — <date>` section to the plan and the packet's brief beside it, reviewed by the brief dimension before dispatch like any brief. Sections already dispatched are not edited, as in any wave.
- The six rules are recomputed for the new packet against every packet of the wave not yet merged and every other claimed wave; a packet that shares a file with one still open waits for that merge.
- Each packet's `## Landed` row, its commit hashes and its project-note line are written in the knowledge pass after its merge; the wave lands, with its retrospective over every packet, when the user closes it.
