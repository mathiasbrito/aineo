...
- **The session's working directory is the main checkout.** A session moved into a clone or a worktree creates every later agent's worktree there, where `.worktreeinclude` finds nothing and the hooks read the wrong branch. Move it back before the next dispatch. **Read an agent's worktree with `git -C <path>` or absolute paths, never `cd`**: a `cd` to a path inside the checkout — an agent's worktree is one — moves the session's own working directory, while a `cd` outside it is reset after the call (observed during wave 2's planning, 2026-09-24; moved back at once).
