...
**A small fix — only when the user calls a packet one** (the user, 2026-09-25: "whenever I call for a small-fix we use the new small-fix rules"). You never label a packet a small fix yourself. Every rule of §3–§7 binds it except these:
- **What qualifies.** One behaviour, changed in one module home — `lua/aineo/<home>/`, `plugin/aineo.lua` outside the autostart, or `doc/` — with its tests, and no new D# or C# row. The Claude session, the MCP server and relay, anything that starts, signals or waits on a process, the autostart, and the shared test harness (`scripts/`, `tests/helpers/`) never qualify: every wave's attack review broke something there. When a called small fix reaches one of them, say so at intake, naming the file, and propose a regular packet; the user decides. An implementer whose fix outgrows the class stops at a green, pushed state and reports a true partial.
- **Two reviews, in one message.** `guarantee` — attack and test-integrity as one review, as for a test-only change — by the domain specialist's implementing type (`neovim-lua-developer`, effort high) bound by the reviewer charter, and `records` by `reviewer`. A documentation-only small fix takes §6's records and reader. The brief review stays: the brief is still the one artefact nothing else re-measures.
- **Mutant survivors are re-run on the change's test files, not the whole suite.** Each mutant still runs first against a narrowed copy of its test file. The implementer's whole suite still runs green before its pull request, and your verification still runs it on the pristine head and on the head laid over `dev`.
- **No re-measure unless a mechanism changed.** After the fix round, dispatch a re-measure only in §6's attack-question cases — the round replaced a mechanism, added a read to a guard, or changed a rule's predicate. Otherwise read the round's report against the findings; your verification re-runs every survivor the reviewers reported, as their literal edits.
- **Labelled.** The brief, the pull request title (`Small fix: …`) and the retrospective say it ran as a small fix, so a defect that gets through is traceable to the lighter review.
