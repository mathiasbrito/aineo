...
| **Effort** | **by role** (the user, 2026-09-24): implementers `high`, reviewers `xhigh` | the definitions pin it — `implementer`, `neovim-lua-developer` and `neovim-claude-code-integrator` carry `effort: high`; `reviewer`, `neovim-lua-reviewer` and `neovim-claude-code-reviewer` carry `effort: xhigh`. A specialist implements under its own name and reviews under its reviewer name; an `Agent` call cannot set effort, so the type chosen is the effort chosen — except a small fix's `guarantee` review (§3), which the implementing specialist takes under its own name at `high`, bound by the reviewer charter (the user, 2026-09-25, accepting the orchestrator's proposal of "normal effort instead of maximum" for small fixes) |
...
- **The session's working directory is the main checkout.** A session moved into a clone or a worktree creates every later agent's worktree there, where `.worktreeinclude` finds nothing and the hooks read the wrong branch. Move it back before the next dispatch. **Never a bare `cd` into a worktree**: read one with `git -C <path>` or absolute paths, and run a command that needs a worktree as its directory in a subshell, `(cd <path> && …)`. A bare `cd` into an agent's worktree moved this session's own working directory during wave 2's planning (2026-09-24; moved back at once); a subshell's `cd` left it in place (measured the same day). The rule held by memory alone and slipped twice more (2026-09-24 17:07, 2026-09-25 01:29), so `.claude/hooks/guard-worktree-cd.sh` refuses the shapes it can read: a `cd` or `pushd` written with `.claude/worktrees` in its argument, at command position in a session's command, is denied — a subshell's is not, a heredoc body or a quoted string is prose, and a subagent's is never checked. It cannot see a directory reached through a variable, a symlink, `CDPATH` or `..`: the rule still binds there.
...
2. **File sets disjoint** — the paths the tasks declare, **plus every registration file a packet would append to**: a module's entry point it exports through, a framework's registration list, a test-support index, the import block above any of them — **and every pin in another suite that counts what the packet adds**, every fixture that builds a body the packet changes, and every pin over a whole served document. Two packets appending to the same list conflict on rebase however different their files. A registration file is checked as a *whole file*: a merge check (`git merge-file`) runs on each packet's complete hunk set against the other's, never on the list the brief happened to name. **One measured exception:** two packets may edit one Markdown document — or one vimdoc help file, whose sections are its `*tag*` headings (measured 2026-09-25 on `doc/aineo.txt`: `git merge-file` of edits under `*aineo-report*` against edits under `*aineo-commands*` and `*aineo-keys*` merged clean) — when each owns a named section, every hunk stays inside its section, at least one unchanged line separates the two packets' nearest hunks, the fence is stated by section heading or tag (never by line number) — for a vimdoc help file, by each section's first and last line, quoted, since a tag on its own line marks no end and a `Heading ~` can sit above the tag — and the second packet re-runs the merge check against the first's *current* head before pushing. For a vimdoc help file that check is also the file's own pin run on the merged file (`make test_file FILE=tests/test_doc.lua`): `git merge-file` cannot see that both sides added one tag, which `:helptags` then refuses (E154).
...
**Size a packet to one component where it can be.** A packet that carries two homes carries both into one context, and its author then carries it through the fix round: T5 held the MCP server and the report (23 files) and its author's context passed 960 K tokens mid-round — the orchestrator's commitment to the user on 2026-09-24. Size the wave to the work, not to the limit: one packet needs one agent. Spawning agents for their own sake is the first failure multi-agent systems are known for.
...
**Ownership, so parallel pull requests do not collide on rebase:** implementers mark only their own task lines and write their own session note. The project note, its changelog row and every commit hash are **yours**, written once per wave on a `knowledge/` branch after the merges (§7) — once per packet in a rolling wave (below).

**A rolling wave — packets specified with the user one at a time** (the user, 2026-09-25, for a wave of small fixes). Every rule of §3–§7 binds each packet, except where this list says otherwise:
- **Intake.** The user describes a problem. You reproduce it — by hand or with an existing suite, in a detached worktree at `origin/dev` that you release afterwards (§9) — and propose the packet: the behaviour it must have, the property its failing test must pin (the implementer writes that test and sees it red), its boundary, and whether it corrects code against a row or changes a decided behaviour, which takes a new D# or C# row superseding the old one through a converge round, never an edit of the old one. Close it with `AskUserQuestion`.
- **The task row.** The agreed packet takes the next free T# as a new row, `active`, in the task list — the *Implementation plan* table of the Planning note the root `CLAUDE.md` names — added in the same `knowledge/` pull request as its section and brief. Every packet of a rolling wave holds its mark (rule 6); the knowledge pass after its merge marks it done.
- **The first plan** lands with its first packet and `rolling: true` in its frontmatter. Its pull request also amends `knowledge-vault/Implementation/Waves/CLAUDE.md` — the `claimed` row, the rule that nothing is edited after dispatch (to admit a later packet's section, brief and brief review, and a re-claim), and the naming list — and adds `rolling: false` to `Templates/Wave Plan Template.md`.
- **A later packet** is one `knowledge/` pull request adding a dated `## Packet <T#> — <date>` section directly above `## Landed`, the packet's brief, and its brief review as `brief-review-<slug>.md` — reviewed by the brief dimension and merged before dispatch, like any brief. The section carries the packet's six-rules row with each rule recomputed by command against every open packet and every claimed wave, the baseline on the `origin/dev` sha it was checked on, the question put to the user and the answer, the reviewer allocation and the verification mutants. Nothing else already in the folder is edited, except the frontmatter's claim and status and `## Landed`.
- **Inside the wave, rule 1 and rule 2 wait for a merge, not for the next wave.** A packet whose prerequisite is in, or that shares a file with, a packet still open waits for that merge; before it is dispatched, every fact in its brief is re-checked against the new `origin/dev`, and a changed fact is a dated amendment (§4).
- **The claim.** The wave stays `claimed` between packets, with nothing running. Only the session named in `claimed_by` adds or dispatches a packet. A later session that continues the wave first re-claims it: having checked that no agent of the old session is still running, it rewrites `claimed_by` and `claimed_at` in a `knowledge/` commit merged before its first `Agent` call. Any other session on the host waits, as §2 says.
- **After each merge**, not once per wave, a knowledge pass writes the packet's `## Landed` row, its commit hashes, its project-note line, and its ledger lines into the wave's retrospective in `Sessions/` — begun at the first packet, extended at each — since a rolling wave can outlive the session and its scratchpad.
- **Closing.** The user closes the wave when no packet is open. A packet planned but not dispatched moves to a later wave's plan, and its section says so. The wave then lands, its retrospective completed over every packet.

**A small fix — only when the user calls a packet one** (the user, 2026-09-25: "whenever I call for a small-fix we use the new small-fix rules"). You never label a packet a small fix yourself. The user calls it before the brief is reviewed, and the brief names it; a packet dispatched as regular stays regular. Every rule of §1 and §3–§7 binds it except these:
- **What qualifies.** One behaviour, changed in one module home — one folder `lua/aineo/<home>/`, `plugin/aineo.lua` outside the autostart (`start_up` and what it reaches: `has_startup_task`, `is_bare_interactive_start`, `open_unless_session_restored`, `open_after_dashboards`), or `doc/` — with its tests, and no new D# or C# row. Never: every file §4's first row gives `neovim-claude-code-integrator` (today `lua/aineo/claude/`, `lua/aineo/mcp/`, `lua/aineo/send/`, the fake `claude` and the recorded transcripts), anything that starts, signals or waits on a process (`lua/aineo/health.lua` runs `claude.cmd`), `lua/aineo/init.lua`, and the shared test harness (`scripts/`, `tests/helpers/`, the `Makefile`) — every wave's attack review broke something there. When a called small fix reaches one of them, say so at intake, naming the file, and propose a regular packet; the user decides. An implementer whose fix outgrows the class stops at a green, pushed state and reports a true partial; the partial goes to the user, who narrows it back or makes it a regular packet, which then takes §6's reviews on its current head.
- **Two reviews, in one message.** `guarantee` — attack and test-integrity as one review, as for a test-only change — by the domain specialist's implementing type (`neovim-lua-developer`, effort `high`) bound by the reviewer charter and the `guarantee` block of `prompts/reviewer-brief.md`, and `records` by `reviewer`. A documentation-only small fix takes §6's records and reader. The brief review stays: the brief is still the one artefact nothing else re-measures.
- **Mutant survivors are re-run on the test files the pull request adds or modifies, not the whole suite.** Each mutant still runs first against a narrowed copy of its test file. The implementer's whole suite still runs green before its pull request, and your verification still runs it on the head and on the head laid over `dev` (§6). A mutant killed only by a test file the pull request does not touch is then reported surviving; it costs the fix round a pin or a measured refutation, and lets no defect through.
- **No re-measure unless the round moved a mechanism.** Dispatch §6's re-measure only when §6 would give it the attack question: the round added a read to a guard, replaced a mechanism, or changed the predicate of a rule that enforces a binding principle. Otherwise read the round's report against the findings yourself. In your verification, run every survivor the reviewers reported and the plan's verification mutants — at least two — each as its literal edit, shown applied, against the whole suite. A claim of the round that your reading or verification refutes goes to §6's bounded correction, with your verification output in place of the re-measure report.
- **Labelled, and what it gives up.** A small fix gives up the separate attack review at `xhigh` and, when no mechanism moved, the re-measure. The brief, the pull request title (`Small fix: …`, never a commit subject — the root `CLAUDE.md` forbids calling a change small) and the retrospective say it ran as a small fix, so a defect that gets through is traceable to the lighter review.
...
**Agent worktrees start from `main`**, the repository's default branch, whose `.claude/` predates `dev`'s (measured in wave 2: every `worktree-agent-*` branch pointed at the bootstrap `511e289`, and one implementer read a stale charter). Every brief and every dispatch prompt therefore tells the agent to check out its branch or head **before** reading anything under `.claude/`.
...
**Choose the agent type by the packet's files**, because a specialist carries the domain facts a generalist has to be told. One row per specialist in `.claude/agents/`:
  ...
  A packet spanning two domains goes to the one whose files carry the risk, and the other domain's specialist takes a review dimension. A new specialist adds its row here in the same `ai/` change that adds its definition — and its reviewer variant, a thin definition pinned at `effort: xhigh` that binds the specialist's file unchanged, so its reviews run at the effort reviews are given. Every specialist reads the matching charter first and is bound by it; the brief's first line says so ("Your role: implement").
...
For each pull request, dispatch reviewers **in one message**, one per dimension from `prompts/reviewer-brief.md`: **attack**, **test-integrity**, **records**. **Two when the change is test-only** — attack and test-integrity collapse into one `guarantee` review whose subject is the guarantee and whose instrument is the tests. **A documentation packet** takes **records** and **reader**. **A small fix** takes the two §3 gives it, of the types §3 names. The type is `reviewer`, or the reviewer variant of the specialist whose domain the pull request touches — `neovim-lua-reviewer`, `neovim-claude-code-reviewer` — bound through the reviewer charter ("Your role: review, dimension <x>"); attack goes to the security specialist's reviewer variant, when there is one, whenever the change touches a guarantee; **a change to the Claude Code integration takes `neovim-claude-code-reviewer` on attack** — it carries the token and permission threat model — and `neovim-lua-reviewer` on test-integrity; **any other change to the plugin's Lua, its vimdoc or its test harness takes `neovim-lua-reviewer` on at least one dimension** — attack or test-integrity, by what the change risks. A re-measure takes the reviewer variant of the implementing specialist. Records is usually the generalist's. Never two specialists of one domain on one pull request. Each brief carries the PR number, head sha, the path of the author's report file, the files, and a resource name `review_<dimension>_<slug>`. **A brief a script generates is read once per pull request before it is dispatched**, and a number in a review brief is measured on the head it names.

**Wait for all of them before anything changes.** Then one round: send the findings to the implementer with `SendMessage` — **unless the implementer's context is past 400 K tokens** (the last request's input and cache tokens, read from its transcript with `.claude/scripts/agent-context.py`), in which case a fresh agent of the same type takes the round, its handover the fix-round file, the author's report and notes, and the branch (the orchestrator's threshold, set on 2026-09-24 after the user's concern about an agent whose context had reached 964 K tokens — T5's author, mid-round; not a measured limit) — built from `prompts/fix-round.md` — every finding, labelled, with the rules that govern the round. **The reviewers' reports travel verbatim as files:** extract each reviewer's final message by machine into `<scratchpad>/<dimension><n>-verbatim.md`, assemble the round into `<scratchpad>/orch-fixround-<n>.md`, and send the path with the rules and your decisions — and **the path of the report you expect back is absolute**. The only exception to waiting is a live security defect, which goes back at once.
...
REVIEWS
| PR | class | attack | test-integrity | guarantee | records | reader | fix round | re-measure | correction | verification |
