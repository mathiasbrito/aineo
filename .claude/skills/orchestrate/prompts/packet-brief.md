...
- **A document shared under rule 2's section exception** (omit when none): <the document; the section this packet owns, named by its first and last line, quoted; the other packets that edit it and their sections>. Every hunk stays inside your section. Before you push, merge your copy with the other packet's current head (`git merge-file`, or `git merge-tree --write-tree <your head> <its branch>`) and run the document's own pin on the merged file (for `doc/aineo.txt`, `make test_file FILE=tests/test_doc.lua`), and report both results.
