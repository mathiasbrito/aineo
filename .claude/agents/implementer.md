...
- **A mutant runs against the tests that target it, not the whole suite.** Run it against the test file — or a copy of that file narrowed to the group that exercises the edited unit — and run the whole suite once, on the final head. A mutant that survives a narrowed run is run against the whole suite before it is recorded as surviving: another file may kill it. Why: in aineo's wave 2 a fix round ran 48 mutants one after another against a whole file of 53 cases that start real processes, for 90 minutes; its correction ran each against the group it targets.
