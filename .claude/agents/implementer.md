...
- **A mutant runs against the tests that target it — by default a copy of its test file narrowed to the group that exercises the edited unit**, not the whole file and not the whole suite; the whole file serves only when it is that group. Every mutant that survives is re-run against the whole suite before it is recorded as surviving: another group or file may kill it — or, when your brief names the packet a small fix (orchestrate §3), against the test files the pull request adds or modifies. This is about mutant runs only — the whole suite still runs green as each unit lands. Why: in aineo's wave 2 a fix round ran two loops of 44 mutants, one after another, each against a whole file of 53 cases that start real processes — about 90 minutes a loop; its correction ran each mutant against the group it targets.
