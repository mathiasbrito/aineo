...
- **Read back only what decides your next step.** A run's summary line and the names of its failing tests, never its whole output; a report or a long file by the section you need (`grep`, `sed -n` ranges), not whole. Everything you read stays in your context for every later turn, and a context past about 400 K tokens degrades the work — the orchestrator hands a round that large to a fresh agent (orchestrate §6).
