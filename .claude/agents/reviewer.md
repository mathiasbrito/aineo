...
A specialist definition in `.claude/agents/` — today `neovim-lua-developer` and `neovim-claude-code-integrator` — binds itself through this charter when its brief names a reviewing role: it reads this file first, this file governs the process and the report, and the specialist's file adds the domain's attack surface and traps. Nothing in a specialist's file relaxes anything here.
