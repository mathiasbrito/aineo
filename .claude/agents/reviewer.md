...
- **Batch what is independent; keep what is evidence as its own step.** Independent read-only checks go in one tool call; a mutant's apply → run → restore stays sequential, and runs against the tests that target it — a narrowed copy of the file when the file is slow — while a survivor is re-run against the whole suite before it is reported surviving. Load what your dimension needs: the records dimension reads the rules, not the coding skills; every dimension reads the spec *section* the brief names, not the artefact whole.
