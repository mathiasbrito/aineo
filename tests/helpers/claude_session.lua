...
--- settings overridden by `overrides`, and returns what `start_session()`
--- returned there.
---
...
--- The fake's first record entry — its arguments, working directory and
--- variables — once it has started, waiting for it at most `PATIENCE_MS`; an
...
function M.wait_for_start(fake)
  vim.wait(M.PATIENCE_MS, function()
    return #M.record(fake) > 0
  end, 20)
  return M.record(fake)[1] or {}
end
