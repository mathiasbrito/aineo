...
Driving the interactive `claude` TUI from a Neovim terminal (aineo's D2) rests on screen text and key behaviour that no document promises — so every fact is a measurement with a version. On Claude Code 2.1.281 in Nvim 0.11.6: **readiness** is the prompt glyph `❯` with no trust-dialog text on screen, then a short settle before input (the trust dialog's `❯ No, exit` carries the same glyph, and the footer `? for shortcuts` can be displaced by a warning); **a bracketed paste** lands unsubmitted and Enter sends it as one message; **stopping** takes keys in order — one Ctrl-C ends a running turn and keeps the process, a double Ctrl-C 0.3 s apart exits 0 at idle but **not during a turn**, presses 1.2 s apart do not exit, `/exit` exits 0, `jobstop` (SIGHUP) exits 129; **`--mcp-config`, `--allowedTools` and `--append-system-prompt`** take effect interactively.
