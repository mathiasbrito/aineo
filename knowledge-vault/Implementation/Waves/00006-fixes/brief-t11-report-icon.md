...
### The behaviours — IC1 to IC5 one test each, each seen red first; IC6 and IC7 are invariants

- **IC1 — the icon by status.** A report's header is `<icon> HH:MM [status] task — summary`: the icon of its status, one space, then the header as it is today.
...
- **IC3 — the time and the status keep their colours at their new columns.** T9's RC1 to RC3 hold on the new header:
  ...
  T9's column pins in `tests/test_report_colours.lua` are moved to the new columns, never deleted.
- **IC4 — the details stay under the status.** Each details line is indented by the display width of what comes before `[status]` in its header: `<icon> HH:MM `.
  - Under Neovim's default `'ambiwidth'`, `single`, every icon is one cell, so that width is 8.
  - Under `'ambiwidth'` `double`, `◐` is two cells and the others stay one (`evidence/icon-widths.txt`), so a `progress` report's details are indented 9.
  - The width is taken when the report is rendered. A later change to `'ambiwidth'` shows in the next rendering, which is the next report, or `:edit` in the Report.
...
- **IC5 — every path shows the icon.** Three paths show it:
  ...
  One render serves all three today: `render.render_records()`, at `lua/aineo/report/init.lua:117` (saved records) and `:179` (a new report). A test on a path other than a new report's shows that it still does.
- **IC6 — the rest of the line is unchanged (an invariant).** The time, the status, the task, the summary and the text of the details are what they were. The saved records' format is unchanged (`lua/aineo/report/records.lua` is outside your boundary), so reports saved before this change show with the icon.
  - Each pin you change over a rendered line changes in exactly two ways: the icon and its space before the header, and a details line's indent.
...
- **The pins over a rendered header**, from `git grep -n -E "\[(started|progress|blocked|done|failed)\] " origin/dev -- tests`:
  - `tests/test_report_buffer.lua`: lines 45, 59, 75, 87, 164, 176, 191, 236, 414, 415, 434, 587, 711, 772 and 800. Its details pins at lines 60–61 indent by six spaces.
  - `tests/test_entry_report.lua`: lines 48 and 70.
    - Its `REPORT_LINES` (lines 12–16) replaces a leading `^%d%d:%d%d ` with `HH:MM `.
    - That pattern stops matching once the icon leads the line. The case at line 48 then fails, on the real time.
  - `tests/test_mcp_delivery.lua`: lines 142, 181, 229, 248 and 268.
  - `tests/test_mcp_blocked_editor.lua`: lines 49, 94 and 95.
  - `tests/test_report_colours.lua`: the column pins at lines 78, 83–87 and 101.
  - Line numbers are `dbc96c9`'s. T13 (PR #31) changes the two `test_mcp_*` files, so re-read every line on your branch.
- **The help.** `doc/aineo.txt` › `*aineo-report*` (lines 253–305):
...
- **Your base is `origin/dev` after T13 merges.** The orchestrator dispatches you only then, and gives the counts measured on that sha in the dispatch message. On it, both versions must be green.
...
- **A document shared under rule 2's section exception:** `doc/aineo.txt`.
  ...
  - **The other packets:**
    ...
    - T12, after T14, edits `*aineo-commands*`, `*aineo-mappings*` and `*aineo-keys*`.
...
- **Asked which icons**, the user chose "Unicode set" — `▸` started, `◐` progress, `⊘` blocked, `✓` done, `✗` failed — over the other options offered. That is C10.
  - The option described the icons as "each one column wide". That holds only under the default `'ambiwidth'`; IC4 is measured.
