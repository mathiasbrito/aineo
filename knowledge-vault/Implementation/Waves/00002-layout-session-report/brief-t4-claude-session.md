...
- **S6 Restart (R1).** Restarting an exited session starts a new process in a new terminal buffer, which the caller receives to show in the left window; the old terminal buffer is wiped (the orchestrator's reading: one Claude buffer at a time, so the buffer list does not fill with dead terminals) — **after** the new buffer has taken its place in every window showing it (`vim.fn.win_findbuf(old)`): *measured by the brief review*, wiping a buffer shown in a window closes that window, which would take the layout's left column with it. Test: a window showing the dead buffer shows the new one after the restart and is still valid. Restarting a running session starts nothing.
- **S7 Stop on quit (R2, Q4).** When Neovim quits with a session running, the session stops `claude` **by the keys a user would press, before any signal** — and none outlives Neovim. R2's row says "interrupting first (SIGINT)": for the interactive TUI, interrupting is the Ctrl-C key written to its terminal (Q4, resolved 2026-09-24) — the byte `\3`; a SIGINT signal to the process was not measured. *Measured* on 2.1.281 (`evidence/t4-summary.txt`, `evidence/t2-summary.txt`):
  - one Ctrl-C (`\3`) **during a turn** ends the turn and keeps the process; the message's text is put back into the input box; the one measured success then waited **2.5 s** before the double press — how soon the turn ends after the first press was not measured;
  - a double Ctrl-C (two `\3`, 0.3 s apart) **at idle** exits 0 — measured with text in the input box and after an interrupted turn: 2843 and 1939 ms after the **first** press of the pair (`evidence/t4-screens.txt`), so about 1.6–2.5 s after the second; presses 1.2 s apart do not exit (`evidence/t2-run2-screens.txt`); `/exit` then Enter exits 0 (`evidence/t2-handshake-out.txt`). The empty-prompt, Ctrl-D and `/exit` timings in `t2-summary.txt` come from a run whose record was not kept;
  - a double Ctrl-C **during a turn** does **not** exit (no exit within 10 s);
  - `jobstop` (SIGHUP to a terminal job) exits 129.

  So the sequence is: one `\3`; then a double `\3`; then a bounded wait for the exit; then `jobstop` as the fallback. The waits are **timings on the orchestrator's host**; derive yours from the phases (the turn ending after the first press, the double-press window, the exit after the second), state them in the report, and keep the whole stop bounded — a quit that hangs is a defect. Register the stop yourself when a process starts (the orchestrator's reading: the child's lifetime is the session's concern, so no caller can forget it). *Measured by the brief review:* `:qa` without `!` quits Nvim 0.11.6 at once while a terminal job runs — `QuitPre` and `VimLeavePre` fire, no E948 or E37 — so the required behaviour is that `:qa` quits the editor and stops Claude by the sequence above. **Every test pays the stop at teardown:** mini.test's `child.stop()` fires `VimLeavePre` in the child and blocks until the handler returns (the brief review measured 3044 ms for a 3 s handler), so a test that leaves a session running spends the whole stop — keep the fake's exit fast, and stop sessions explicitly where the test allows.
- **S8 The fake `claude` (C3, D10).** An executable under `tests/helpers/` that the suites run in the CLI's place — through `claude.cmd`, so no test depends on `PATH` order. It records, to a file the test names through its environment, its argv, the environment variables a test asks about, and every byte it receives, and it plays the behaviours of S4 and S7 **as measured**, not as documented:
  - *ready*: replays the recorded startup bytes (`evidence/t4-claude-2.1.281-startup-paste-exit.bytes.txt` — copy what you use into your fixtures with the version and date in a header; the file's claude.ai Remote Control link is replaced by a placeholder, and any link you find stays out of the fixtures); then echoes what it receives, and exits 0 on a double `\3` at idle, as the real CLI did;
  - *trust*: shows the trust dialog's text (`evidence/t4-trust-dialog-screen.txt`) and never becomes ready; it exits 0 on a double `\3` too;
  ...
  **A pty delivers Ctrl-C as SIGINT unless the program puts its terminal in raw mode** — the real `claude` does (its bytes enable bracketed paste, `ESC[?2004h`). A fake that stays in cooked mode receives a signal, not the byte `\3`; measure what yours receives. The recorded bytes also carry terminal queries whose answers Neovim's terminal writes back to the fake's input — *measured by the brief review* with a raw-mode stand-in replaying these bytes in Nvim 0.11.6: XTVERSION (`ESC P>|libvterm(0.3) ESC \`), the kitty keyboard flags (`ESC[?5u`), DA1 (`ESC[?1;2c`), DECRQM (`ESC[?2026;0$y`, `ESC[?1016;0$y`) and cursor-position reports (`ESC[?<row>;<col>R`, which depend on the terminal's size); nothing answers `ESC[16t` or the kitty graphics query. Tests that assert on received bytes account for them. The same stand-in received `\3` as a byte in raw mode and was ended by SIGINT (exit 130) in cooked mode. Write the fake in Lua run by Neovim (`nvim -l`) or in POSIX `sh` — the suite needs no other interpreter.
...
- **M4** — build the `--mcp-config` entry's `env` with `{}` where it is empty (it then encodes as `[]`) → the S2 test fails — by its assertion on the raw text (S2).
- **M5** — drop the trust-dialog condition from readiness (the glyph alone) → the S4 *trust* test fails.
- **M6** — remove the first, single `\3` from the stop sequence → the S7 *busy* test fails. The fallback still stops the fake under M6, so "no `claude` outlives Neovim" does not kill it: the test asserts the fake's own record of how it ended — exit 0 on keys, no SIGHUP.
