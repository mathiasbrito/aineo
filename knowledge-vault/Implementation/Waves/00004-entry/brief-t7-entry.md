...
- **EP4 Open (`\o`)** — the composition:
  - the configuration resolved (`require('aineo.config').resolve_config(vim.g.aineo, recorded_setup_options())`): a wrong value is one error message naming the setting, and nothing opens; unknown keys raise nothing here — `neovim-lua-developer.md` sends them to the health check, not the hot path, so T8 reports them;
...
- **EP6 Send (`\s`)** — `require('aineo.send').send()` (T6: no argument; it refuses with its own message). **Not an `<expr>` mapping:** Send clears Input before it writes, and under textlock — an `<expr>` mapping's evaluation — the clear fails and `send()` raises E565, writing nothing (T6's fix round; the brief review of wave 4 measured the error).
...
- **EP8 A startup dashboard (D15).** When a dashboard shows at a bare start — snacks.nvim's, alpha, dashboard-nvim, mini.starter — aineo's layout still opens, and the dashboard is not left on screen; with `autostart = false` the dashboard is left alone. The orchestrator measured no dashboard. **mini.starter is in `deps/` already** (mini.nvim at its pin) — use the real one: *measured by the brief review*, it opens under `noautocmd` (`deps/mini.nvim/lua/mini/starter.lua`, its open path), so a stand-in that fires `FileType` would pass where the real one fails. For the others read when each opens from its source at a version you cite (raw source, never a summary), or build stand-ins that open a buffer of each one's filetype at `VimEnter`, at `UIEnter` and from `vim.schedule()` after `VimEnter`, with and without autocommands; the property holds for every moment you pin, and the note says which you did not.
- **EP9 End to end through the fake.** A new mode of the fake `claude` acts as the MCP client Claude Code is: it reads its `--mcp-config` argument, starts the server entry it names (command, arguments, environment), speaks `initialize`, `notifications/initialized`, `tools/list` and `tools/call` of the report tool, and exits. `\o` with that fake shows the report in the Report, rendered in C6's format. This proves the wiring the homes' own suites inject: `v:servername`, `v:progpath`, the entry's environment. *Measured by the orchestrator on the real CLI* (`evidence/t7-summary.txt`): wired by hand as EP4 wires it, Claude Code 2.1.281 accepted the report tool — its schema's `"details": {"type": ["string", "null"]}` included — and its call rendered in the Report. That run used the user's own Claude Code settings, in auto mode, so it shows nothing about `--allowedTools` pre-allowing the tool (T2's control run did, `../00002-layout-session-report/evidence/t2-summary.txt`, Q2).
- **EP10 The plugin file stays cheap.** Sourcing `plugin/aineo.lua` requires no aineo module. At `VimEnter` the plugin may load `aineo.config` — and nothing else — to resolve the prefix and `autostart`: *measured by the brief review*, T1's pin *is sourced at startup and loads no aineo module* reads `package.loaded` after `VimEnter` and fails on a config read there. Move that pin to name exactly what a headless start loads, and route the reading to the MVP list.
...
mini.test's child always starts with `--headless` (`deps/mini.nvim/lua/mini/test.lua`, the child's start arguments), so its `VimEnter` sees no UI. *Measured by the brief review of wave 4* (its probe `test_terminal_route.lua`): an interactive `nvim` run in a terminal job of a mini.test child (`jobstart(…, { term = true })`), queried over its own `--listen` address through `sockconnect`, sees one UI and `argc()` 0 at `VimEnter` with `plugin/aineo.lua` sourced, is still alive 1.5 s later, keeps `stdpath('state')` and `CLAUDE_CONFIG_DIR` under `.tests/`, and exits 0 on `qa!`. Make the child that holds the terminal at least 160 columns wide — 240 when a file column is open — since the fake's screens are 80 columns wide (below). *Not this:* an `nvim --embed` RPC job the test attaches a UI to — measured by the same review, it exits 1 about 10 ms after the attach, because the test's Neovim does not take its `redraw` notifications (`evidence/t7-startup-summary.txt`, item 2, as corrected).

**No Neovim a suite starts may run the real `claude` (D10).** Every child sources `plugin/aineo.lua`, the defaults are `autostart = true` and `claude.cmd = { "claude" }`, and `claude` is on the suite's `PATH`: a broken UI check — M19 itself — would start the real CLI in every child. Before any autostart exists, put a guard in place that makes that impossible — for every Neovim `make test` and `make test_file` start, a `claude` that is not the real one comes first, or the suites' default configuration names the fake — and pin the guard with a test that fails if the real `claude` could run. The mechanism is yours, stated; the `Makefile` and `scripts/minimal_init.lua` are open to you for it and for the next point only. *Measured by the brief review:* an `AINEO_CHILD` in the environment of `make test` reaches every child — run inside aineo's own Claude terminal, the positive autostart tests would refuse and M19 would survive — so the suites remove `AINEO_CHILD` as they remove `CLAUDE*`, and the test for C1's `$AINEO_CHILD` refusal sets it itself.
...
- `origin/dev` at dispatch is `bb0e185` followed by this wave's knowledge commits (the plan and its claim, `knowledge-vault/` only), so its code is `bb0e185`'s: waves 1–3 landed.
...
- **You may touch:** `plugin/aineo.lua`; `lua/aineo/init.lua`; `lua/aineo/config/` for EP11 only; `tests/test_plugin.lua` and `tests/test_aineo.lua` for the pins above and EP11's cases (`setup()`'s group); `tests/test_config.lua` for EP11; the `Makefile` and `scripts/minimal_init.lua` for the guard against the real `claude` and the removal of `AINEO_CHILD` only (*How tests reach a UI-attached start*); `tests/test_entry*.lua` (new); `tests/helpers/fake_claude.lua` and `tests/helpers/claude_session.lua` — new modes and helpers only; new files under `tests/helpers/` whose names begin with `entry`; new fixtures under `tests/fixtures/claude/` (where the fake reads its screens — new files only, each with a header naming its source) and `tests/fixtures/entry/`; and the session note below.
...
- **The prefix is mapped by default** — C1, D1 and D13 are the user's decisions, and they win over `neovim-lua-developer.md`'s general "no global keymaps by default": the prefix mappings are made unless `prefix = false`, and only where the user has not mapped the key sequence. Unknown configuration keys go to the health check, as that file says (EP4).

## Verification mutants — the orchestrator runs these on your final head

- **M19** — autostart without the UI check → the `--headless` test fails: a session started. Run it narrowed, and only once the guard against the real `claude` is in place and pinned.
...
- **M22** — autostart without the `$AINEO_CHILD` check → that test fails. Each refusal case changes exactly one condition of a start that does autostart — UI attached, no file, no stdin, `autostart` true — so M22 dies only by its own case.
- **M23** — autostart skipped while a dashboard's filetype shows → the D15 test fails.

## Readings

Every choice where the rows and this brief are silent goes to a *Readings for the MVP review* section of your session note, each named once, with the same list in the pull request body and your report — among them EP1's message, EP4's error messages, EP5 starting a session when a focus finds none, EP7's unnamed starts, EP8's moments, and what EP10 lets load at `VimEnter`.
