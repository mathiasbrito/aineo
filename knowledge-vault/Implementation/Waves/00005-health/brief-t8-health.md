...
- **HB1 The check.** `:checkhealth aineo` runs `require('aineo.health').check()`, built on `vim.health.start`, `ok`, `warn`, `error` and `info` (`:h health-dev`, Neovim 0.11.6). It starts nothing and changes nothing: no Claude session, no layout, no mapping. The composition belongs to `plugin/aineo.lua`.
- **HB2 Claude and its version.** The first word of `claude.cmd`, from the resolved configuration:
  - not executable → an error naming it, with the advice to install Claude Code or set `claude.cmd`;
  - executable → run it with `--version` as a list, never through a shell, bounded in time (your bound, stated), and report the output as `ok`.
  ...
  - A `--version` that fails, prints nothing or runs past the bound → a warning.
  - **In the suites the real `claude` never runs:** the suites put a stand-in first on `PATH` that exits 127 (`scripts/minimal_init.lua`, `tests/helpers/entry_guard/claude`). A test of the executable case sets `claude.cmd` to a stand-in of your own that answers `--version` — for example a new fake mode, or a script under `tests/helpers/health*`.
...
- **HB4 Prefix-mapping conflicts (C7, R3).** For each of `<prefix>s`, `o`, `r`, `i` and `c` with the resolved prefix:
  ...
  - `maplocalleader` equal to the prefix → a warning that a filetype plugin's `<LocalLeader>` mapping would shadow aineo's keys in that buffer (R3);
  - `prefix = false` → `info`, nothing mapped.
- **HB5 Why the autostart did or did not run (C7).**
  - T7 records no reason today (`plugin/aineo.lua`, `start_up()`), and some of its inputs exist only at startup: stdin read, `v:argv`, a session a plugin restored. Record the decision and its reason where the health check can read it — you may change `plugin/aineo.lua` for that and nothing else.
  - The check reports one reason: it ran; `autostart = false`; no UI attached (headless); a file argument; stdin read; a startup task (`-c`, `-S`, `-e`, `-s`, `-E`, `+cmd`); `$AINEO_CHILD` (inside aineo's own Claude terminal); a restored session; sourced after `VimEnter`; a wrong setting.
...
- `origin/dev` at dispatch is `201873b` followed by this wave's knowledge commits (vault only) and PR #19 (agent documentation only), so its code is `201873b`'s: waves 1–4 landed.
...
- **The suites** (the root `CLAUDE.md`, and PR #19 once merged): `make test` isolates every Neovim under `.tests/`, puts a `claude` that exits 127 first on `PATH`, removes `AINEO_CHILD`, and presets `vim.g.aineo = { autostart = false }` unless a test sets it with `--cmd`. A health test of the autostart reason runs a start that autostarts, or refuses, through T7's interactive-editor helper (`tests/helpers/entry_editor.lua`), with `claude.cmd` naming the fake.
