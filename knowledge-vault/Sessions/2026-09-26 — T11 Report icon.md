...
**Branch:** `feature/t11-report-icon` · **Pull request:** #45 into `dev` (a regular packet), with one fix round
...
- **Fix round.**
  ...
  - Five cases pin what the reviews found unpinned.
...
**Arrived green (8).** Each killer below was run, and each killed by assertion. The mutants are in the table under *Mutants*.
...
- `a report › indents each report's details by its own icon when reports of different widths show together` (fix round, A2 and I1; the attack's A7). Killers: MA1, MA2 and X1.
- `a report › of every status the report tool accepts shows with an icon` (fix round, A4). Killers: MF, a status added to `format.STATUSES` with no icon, which kills this case alone; and MD, an icon dropped.
...
- `make test` on 0.12.5, the host's: 831 cases, `Fails (0)`, exit 0. That is 808 on the base plus the 23 cases this packet adds: 15 in the first round, 8 in the fix round.
- `env PATH=<builds>/nvim-0.11.6/nvim-macos-arm64/bin:… make test` on 0.11.6: 831 cases, `Fails (0)`, exit 0.
