...
**Branch:** `feature/t1-tooling` · **Pull request:** #4 (one packet round, one fix round)
...
**A stall check replaces the time limit.** A `MiniTest.finally` that raises stops mini.test's queue. The run then held for 30 minutes and could end green. Now it fails after 10 s idle, saying so. The design is the attack reviewer's measured runner, adopted red-first.
...
- **What the stall check cannot see.** Neither a case that blocks Neovim itself nor a slow case in its own `vim.wait` is observable to it. mini.test's steps run back to back within one pass of Neovim's event processing (measured, below).

**No child Neovim outlives a stopped run (FR-D1).**

- A stalled runner ends through `cquit`, and Neovim stops its jobs on the way out.
...
- **Seen red:** 15 tests, after three invalid reds, each a harness defect fixed first:
...
- **Arrived green:** 7 tests, each with a named killer:
...
**Mutant table.** Literal edits, one at a time, each file restored byte for byte, all on the fix round's code commit. The merged head carries that code unchanged. The packet round's table was measured on its configuration commit, and this table replaces it wherever the code changed.
...
| no stall branch (MUlimit / R16 equivalent) | delete `if not wait_for_execution() then fail_run(…) end` | 11 tests (assertion) |
...
| R15 (reviewer's literal) | `test: deps` → `test:` | **survived** — a fresh checkout fails loudly without mini.nvim; not pinned, as the reviewer also judged |
...
- **For the `ai/` pass.**
  ...
  - `VIMRUNTIME` still reaches the suite from a parent Neovim (attack review, under A3). Unsetting it would break a developer who runs a development build of Neovim through it. Left for a decision, not changed.
- **For T7.**
...
- **Limits.**
  - A case that blocks Neovim itself — a busy loop, or a request to a child Neovim that never answers — is bounded by nothing in the runner, only by an outside watchdog. The suite's own helper stops such a run's whole process tree.
