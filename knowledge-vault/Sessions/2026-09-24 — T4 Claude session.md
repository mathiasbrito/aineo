...
- **For the knowledge pass (records R6):** the measurements behind the packet's departures (`t4-probe-kill`, `t4-probe-size`, `t4-probe-deaf`, `t4-probe-hupdeaf`, `t4-probe-race`, the `t4-sample-*` stacks, the mutant logs), the fix round's (`t4f-*`), the re-measure's (`remeasure11-*`) and the correction's (`t4c-*`) are in the orchestrating session's scratchpad, not in the wave's `evidence/`, which a packet may not write; they will not outlive the session unless copied.
